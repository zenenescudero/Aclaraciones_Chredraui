...
Public Class facturasmasivo
    ...
    Public Sub guardar()
        ...
        Try
            idinicial = obteneruncampo("select max(id) from facturas")
        Catch ex As Exception
            idinicial = 0
        End Try
        ...
        If CheckBox1.Checked Then
            Try
                conexion.Open()
                Dim coman As New MySqlCommand("delete from facturas where id<=" & idinicial & "", conexion)
    ...
    Public Sub cargafail(ByVal fila As Integer)
        Try
            conexion.Open()
            Dim coman As New MySqlCommand("delete from facturas where id>" & fila & "", conexion)
            coman.CommandType = CommandType.Text
            coman.ExecuteNonQuery()
            conexion.Close()
        Catch ex As Exception
